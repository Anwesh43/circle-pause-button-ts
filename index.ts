...
class CPBNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new CPBNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawCPBNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : CPBNode {
        var curr : CPBNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr != null) {
...
class CirclePauseBtn {

    curr : CPBNode = new CPBNode(0)
    dir : number = 1 
